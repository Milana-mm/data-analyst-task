...
count_by_category_late = merged_df.groupby(['customer_city', 'occupation', 'late']).size().unstack(fill_value=0)
...
# heatmap 2.2
# group by - to get counts
count_by_category_late2 = merged_df.groupby(['library_name', 'education', 'late']).size().unstack(fill_value=0)
# calculate late ratio for group by combination
count_by_category_late2['late_ratio'] = count_by_category_late2.get(1, 0) / (
        count_by_category_late2.get(1, 0) + count_by_category_late2.get(0, 0)
)
# reshape for heatmap: pivot table - x-axis and y-axis
heatmap_data2 = count_by_category_late2['late_ratio'].unstack()

# -----------
# create a subplot
fig = make_subplots(rows=2, cols=2,
                    subplot_titles=("Scatter Plot per Customer City",
                                    "Bar Chart per Customer City"))
...
fig.add_trace(go.Heatmap(
    ...
    x=heatmap_data.columns,  # categories x-axis
    y=heatmap_data.index,  # categories y-axis
...
# Heatmap trace
fig.add_trace(go.Heatmap(
    z=heatmap_data2.values,  # late ratio values
    x=heatmap_data2.columns,  # education categories x-axis
    y=heatmap_data2.index,  # categories y-axis
    colorscale='Pinkyl',  # use a color scale where intensity reflects late ratio
    colorbar=dict(
        title='Late Ratio',
        titlefont=dict(color='white'),
...
# -----------
# Update layout
fig.update_layout(title_text="Data Analyst - Library Analysis",
